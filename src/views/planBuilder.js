...
//
// Navigation is plans-first: the tab lands on a list of every saved
// plan (active one clearly marked), and tapping one opens its options
// screen (details/session types/etc, plus Save/Activate/Delete/export).
// There's no separate "preview" step — export is just two buttons on
// that same screen.
...
export function renderPlanBuilderView({ activePlanId, onPlanActivated, onActivePlanUpdated }) {
  ...
  // 'list' (every saved plan) or 'options' (editing one specific plan,
  // held in `data` — same shape/localStorage-autosave as always).
  let view = 'list'
  let data = null
  // True once *this* plan.id has a real Firestore doc behind it — from
  // opening an existing saved plan, or after this session's first
  // successful Save of a new/imported one. Activate and Delete both
  // need this: Activate always pushes live whatever's actually saved
  // (never unsaved in-memory edits), so there must be something saved
  // to point at; Delete obviously needs something there to remove.
  ...
  function render() {
    root.innerHTML = ''
    if (view === 'options' && data) renderOptions()
    else renderList()
  }

  function openPlan(planDoc) {
    data = importPlanJson(planDoc)
  ...
  function openImportedPlan(raw) {
    data = importPlanJson(raw)
    hasSavedAtLeastOnce = false
    save()
    view = 'options'
    render()
  ...
  function sectionLink(name, count, onClick, opts = {}) {
    const link = el('div', { class: 'pb-section-link' + (opts.active ? ' pb-active' : ''), onclick: onClick })
    ...
    if (opts.active) link.appendChild(el('span', { class: 'pb-active-badge', html: 'Active' }))
  ...
  // ── List: every saved plan ──────────────────────────────────────────

  function renderList() {
    root.innerHTML = `
      <div class="topbar"><span class="topbar-title">Plan builder</span></div>
      <div class="content plan-builder-content" id="pb-list-content"></div>
    `
    const content = root.querySelector('#pb-list-content')
    content.appendChild(hint('Every plan you’ve saved. Tap one to view or edit it — exactly one can be active for training at a time.'))
    ...
    function renderPlansList() {
      listWrap.innerHTML = ''
      cloudDb.getAllPlans().then(plans => {
        listWrap.innerHTML = ''
        if (plans.length === 0) {
          listWrap.appendChild(el('p', { class: 'text-sm text-muted' }, [document.createTextNode('Nothing saved yet — start a blank plan or import a JSON file below.')]))
          return
        }
        ...
        plans.forEach(p => {
          ...
          card.appendChild(sectionLink(
            p.plan?.title || p.id,
            isActive ? undefined : (p.plan?.startDate ? `${p.plan.startDate} → ${p.plan.endDate}` : undefined),
            () => openPlan(p),
            { active: isActive },
          ))
        })
        ...
        errBox.textContent = err.message || 'Could not load saved plans.'
        listWrap.appendChild(errBox)
      })
      listWrap.appendChild(el('p', { class: 'text-sm text-muted' }, [document.createTextNode('Loading…')]))
    }
    renderPlansList()

    const newBtn = el('button', { class: 'btn btn-primary btn-full', type: 'button', style: 'margin-top:16px' })
    newBtn.textContent = '+ New plan'
    newBtn.addEventListener('click', openBlankPlan)
    content.appendChild(newBtn)

    const importLabel = el('label', { class: 'btn btn-ghost btn-full', style: 'cursor:pointer;margin-top:8px' })
    importLabel.textContent = 'Import a JSON file…'
    ...
    importInput.addEventListener('change', async e => {
      ...
      try {
        ...
        openImportedPlan(JSON.parse(text))
      } catch (err) {
  ...
  function renderOptions() {
    root.innerHTML = `
      <div class="topbar"><span class="topbar-title">Plan builder</span></div>
      <div class="content plan-builder-content" id="pb-options-content"></div>
    `
    const content = root.querySelector('#pb-options-content')
    const isActive = !!data.plan.id && data.plan.id === activePlanId

    const back = el('button', { class: 'btn btn-ghost', type: 'button', style: 'margin-bottom:14px' })
    back.textContent = '← All plans'
    back.addEventListener('click', () => { view = 'list'; render() })
    content.appendChild(back)

    const titleRow = el('div', { style: 'display:flex;align-items:center;gap:10px;margin-bottom:16px;flex-wrap:wrap' })
    const titleEl = el('span', { style: 'font-size:22px;font-weight:600;color:var(--text)' })
    titleEl.textContent = data.plan.title || 'New plan'
    titleRow.appendChild(titleEl)
    if (isActive) titleRow.appendChild(el('span', { class: 'pb-active-badge', html: 'Active' }))
    ...
    const validation = el('div', { class: 'pb-validation ' + (valid ? 'ok' : 'bad'), style: 'margin-top:16px' })
    ...
    const saveBtn = el('button', { class: 'btn btn-primary btn-full', type: 'button', style: 'margin-top:16px' })
    ...
    saveBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        hasSavedAtLeastOnce = true
        // Editing the plan that's already live pushes the update straight
        // to the app (no separate Activate needed — it was already
        // active); saving a plan that isn't active just persists it.
        if (isActive) onActivePlanUpdated(exported)
        saveBtn.textContent = 'Saved ✓'
        setTimeout(() => { saveBtn.textContent = 'Save'; saveBtn.disabled = !valid }, 1200)
      } catch (err) {
    ...
    if (isActive) {
      content.appendChild(hint('This is the active plan — saving pushes the update immediately.'))
    } else {
      content.appendChild(hint('Saving doesn’t activate it — use Activate below when it’s ready to train from.'))
    }

    if (!isActive) {
      const activateBtn = el('button', { class: 'btn btn-ghost btn-full', type: 'button', style: 'margin-top:8px' })
      activateBtn.textContent = 'Activate'
      activateBtn.disabled = !hasSavedAtLeastOnce
      activateBtn.title = hasSavedAtLeastOnce ? '' : 'Save this plan first'
      activateBtn.addEventListener('click', async () => {
        activateBtn.disabled = true
        activateBtn.textContent = 'Activating…'
        try {
          // Activates whatever is actually saved for this id, not any
          // unsaved in-progress edits — Save and Activate stay two
          // genuinely separate steps.
          const saved = await cloudDb.loadPlan(data.plan.id)
          if (!saved) throw new Error('That saved plan is gone — try refreshing the list.')
          await cloudDb.setMeta('activePlanId', data.plan.id)
          onPlanActivated(saved)
        } catch (err) {
    ...
    if (hasSavedAtLeastOnce && !isActive) {
      const deleteBtn = el('button', { class: 'btn btn-ghost btn-full', type: 'button', style: 'margin-top:8px' })
      deleteBtn.textContent = 'Delete'
      deleteBtn.addEventListener('click', () => {
        showConfirm(`Delete "${data.plan.title || data.plan.id}"? This can’t be undone.`, 'Delete', async () => {
          try {
            await cloudDb.deletePlan(data.plan.id)
            clearDraft()
            data = null
            view = 'list'
            render()
    ...
    const exportRow = el('div', { style: 'display:flex;gap:8px;margin-top:16px' })
    const copyBtn = el('button', { class: 'btn btn-ghost', type: 'button', style: 'flex:1' })
    ...
    const downloadBtn = el('button', { class: 'btn btn-ghost', type: 'button', style: 'flex:1' })
    ...
    downloadBtn.addEventListener('click', () => {
      ...
      a.href = url
      a.download = (exported.plan.id || 'plan') + '.json'
